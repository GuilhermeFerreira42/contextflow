...
class GridPanel(wx.Panel):
    ...
    def _init_ui(self):
        ...
        self.grid.CreateGrid(0, 9) # Rows autogenerated, 9 Cols
        
        # Col Headers
        cols = [" [x] ", "ID", "Link", "Título", "Playlist/Canal", "Duração", "Tokens", "Custo ($)", "Status"]
        ...
        self.grid.SetColSize(2, 200) # Link (Nova)
        self.grid.SetColSize(3, 350) # Title
        self.grid.SetColSize(4, 150) # PL
        self.grid.SetColSize(5, 70) # Duration
    ...
    def load_data(self):
        ...
        for i, v in enumerate(videos):
            ...
            # 5: Duration
            d = v.get('duration')
            self.grid.SetCellValue(i, 5, str(d) if d else "0")
            ...
            # 7: Custo
            cost = t * 0.000005
            self.grid.SetCellValue(i, 7, f"{cost:.4f}")
            
            # 8: Status
            status = v.get('status', 'pending')
            self.grid.SetCellValue(i, 8, status)
            if status == "ERROR":
                 self.grid.SetCellTextColour(i, 8, wx.RED)
            else:
                 self.grid.SetCellTextColour(i, 8, wx.BLACK)
    ...
    def on_task_queued(self, task_uuid, url):
        ...
        self.grid.SetCellValue(row, 8, "Na Fila")                   
    ...
    def on_task_started(self, task_uuid):
        ...
        if row is not None:
             self.grid.SetCellValue(row, 8, "Baixando...")
    ...
    def on_task_update(self, video_id, status):
        ...
        if row is not None:
            self.grid.SetCellValue(row, 8, status)
        else:
    ...
    def on_task_complete(self, data):
        ...
        if row is not None and video_record:
            self.grid.SetCellValue(row, 8, "completed")
            
            # Update Tokens & Custo & Duration final
            t = video_record.get('token_count', 0)
            cost = t * 0.000005
            
            self.grid.SetCellValue(row, 6, str(t))
            self.grid.SetCellValue(row, 7, f"{cost:.4f}")
            self.grid.SetCellValue(row, 5, str(video_record.get('duration', 0)))
            
            # Reseta cor se estava em erro antes
            self.grid.SetCellTextColour(row, 8, wx.BLACK)
    ...
    def on_task_error(self, video_id, error_msg):
        row = self._find_row_by_id(video_id)
        if row is not None:
            self.grid.SetCellValue(row, 8, "ERROR")
            self.grid.SetCellTextColour(row, 8, wx.RED) # RED status
