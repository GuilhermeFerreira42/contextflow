...
class GridPanel(wx.Panel):
    ...
    def _init_ui(self):
        ...
        # Tamanhos
        self.grid.SetColSize(0, 40)  # Checkbox
        ...
        self.grid.SetColSize(6, 120) # Adicionado (New)
    ...
    def load_data(self):
        ...
        for task in active_tasks:
            ...
            self.grid.SetCellValue(current_row, 6, datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")) # Adicionado agora (aprox)
        ...
        for v in videos:
            ...
            # Adicionado em
            self.grid.SetCellValue(current_row, 6, v.get('added_at') or "")

            self.grid.SetCellValue(current_row, 7, v.get('playlist_title') or "-")
            
            d = v.get('duration')
            # Se duration for int (segundos), formata? Ou o youtube_manager já salvou formatado?
            # O youtube_manager salva str HH:MM:SS no 'duration', e int no 'duration_seconds' se mudamos o add_video.
            # No add_video, passamos tudo o que vem do metadata.
            # Se no banco antigo tiver int, converto na hora se der (try).
            if str(d).isdigit():
                 # Conversão legacy on-the-fly
                 try:
                     from services.youtube_manager import YouTubeManager
                     d = YouTubeManager()._format_duration(int(d))
                 except: pass
            
            self.grid.SetCellValue(current_row, 8, str(d) if d else "00:00:00")
            
            t = v.get('token_count') or 0
            self.grid.SetCellValue(current_row, 9, str(t))
    ...
    def on_task_queued(self, task_uuid, url):
        ...
        # Timestamp Instantâneo
        import datetime
        self.grid.SetCellValue(row, 6, datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
    ...
    def on_task_complete(self, data):
        ...
        if row is not None and video_record:
            ...
            self.grid.SetCellValue(row, 6, video_record.get('added_at') or "")
            self.grid.SetCellValue(row, 7, video_record.get('playlist_title') or "-")

            d = video_record.get('duration')
            self.grid.SetCellValue(row, 8, str(d) if d else "00:00:00")
            
            self.grid.SetCellValue(row, 9, str(t))
            # Cost removed (Col 10 is Status)
