...
class AppWindow(wx.Frame):
    ...
    Integra as 3 abas independentes e o monitoramento global de performance.
    """
    ...
    def _bind_events(self):
        ...
        pub.subscribe(self.on_global_progress, 'TASK_PROGRESS')
        pub.subscribe(self.on_global_error, 'TASK_ERROR')
    ...
    def on_global_error(self, video_id, error_msg):
        """Alerta global sobre falhas (ex: Banimentos IP 429) [3, 7]."""
    ...
    def on_reprocess_errors(self, event):
        ...
        if error_urls and wx.MessageBox(f"Reprocessar {len(error_urls)} erros?", "Confirmação", wx.YES_NO) == wx.YES:
            pub.sendMessage('REQUEST_BATCH_PROCESSING', urls="\n".join(error_urls))